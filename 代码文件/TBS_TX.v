module TBS_TX #(
    ...
    localparam PULSE_WIDTH_COUNT = BIT_PERIOD_COUNT / 8; // 脉冲宽度定义为比特周期的 1/8
    ...
    always @(posedge clk_50M or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (start_of_frame_detected) begin
                ...
                // --- 特殊处理停止位 (9) ---
                // 不需要等停止位完全结束，只要过了一半时间(BIT_PERIOD_COUNT/2)
                // 就认为结束，提前释放 tx_active 以捕捉下一个紧邻的起始位
                else if (bit_cnt == 9) begin
                    if (baud_cnt >= BIT_PERIOD_COUNT / 2) begin
                        tx_active <= 1'b0;
                        bit_cnt   <= 0;
                    end
                end
    ...
    always @(posedge clk_50M or negedge rst_n) begin
        ...
        else if (pulse_cnt < PULSE_WIDTH_COUNT) begin
